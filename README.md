...
本站把羊毛集中到一页：**鸡蛋榜**（谁在送、送多少、几号过期）、**免费榜**（真·免费的版本与额度）、**套餐榜**（订阅横评）、**模型榜**（按量单价速查）。
...
### 方式一：手动改数据
打开 `data.js`，改 `eggs / free / plans / models`，刷新页面即可。倒计时、临期预警、过期置灰都会按 `end` 日期自动重算。
...
2. 逐条检查鸡蛋榜 + 免费榜现有链接（404/410 判定失效，403/超时视为「无法验证」不定罪）；
3. 价格与时效自动化：国际模型价格每天从 OpenRouter 公开 API 对表（偏差 >15% 自动更正并在报告标注）、DeepSeek 官方定价页每日核验、过期超 30 天的条目自动移除（`--apply` 生效）；
4. 生成当日巡检报告。`--apply` 会把「同时带金额/额度和截止日期」的新活动写回 `data.js`，
   打上 `auto: true` 标记，页面上显示「自动收录」角标，供人工复核。

准确性边界：套餐榜与国产模型价格没有可靠的公开结构化数据源，为保证准确**不会自动改动**，
报告会把它们列进人工巡检清单，点开官网核对即可。
...
之后仓库的 Actions 会：
- 每天北京时间 09:17 / 21:17 两次自动运行巡检脚本（活动收录、死链检查、价格对表、过期清理），有变更就提交 `data.js` 和报告；
